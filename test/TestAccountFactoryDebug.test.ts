...
describe("TestAccountFactory", function () {
  ...
  beforeEach(async function () {
    ...
    // Deploy the TestAccountFactory (original)
    const TestAccountFactory = await ethers.getContractFactory("contracts/TestAccountFactory.sol:TestAccountFactory");
  ...
  it("should emit an AccountCreated event with the correct arguments", async function () {
    const salt = 3;
    const expectedAddress = await factory.getAccountAddress(userAddress, salt);
...
describe("TestAccountFactoryDebug", function () {
  ...
  it("should correctly predict the account address", async function () {
    ...
    const predictedAddress = await factory.getAccountAddress(userAddress, salt);
    console.log("getAccountAddress result:", predictedAddress);
    ...
    expect(createDebug.actualAddress).to.equal(createDebug.predictedAddress);
    expect(createDebug.predictedAddress).to.equal(debugInfo.predictedAddress);
  });
});