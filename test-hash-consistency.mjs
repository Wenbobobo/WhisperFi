...
async function testFrontendPoseidon() {
    ...
    const testCases = [
        ...
        [BigInt("8175042333908131853555108599311849679722172756805630201899011284758317870395"), BigInt(0)] // 从log中的commitment
    ];
...
async function testMerkleTreeConstruction() {
    ...
    const commitment = "8175042333908131853555108599311849679722172756805630201899011284758317870395";
    ...
    for (let level = 0; level < 20; level++) {
        ...
        if (level < 5) { // 只显示前几层避免输出过长
            console.log(`  计算: poseidon([${level === 0 ? commitment : 'previous'}, 0]) = ${currentHash.toString()}`);
        }
    }
