...
const tokens = [
  ...
  },
  {
    ...
    logo: "https://cdn.jsdelivr.net/gh/atomiclabs/cryptocurrency-icons@1a63530be6e374711a8554f31b17e4cb92c25fa5/svg/color/dai.svg",
  },
  {
    symbol: "USDC",
...
export default function TradeCard() {
  ...
  const handleGetQuote = async () => {
    ...
    try {
      ...
      setFeedback({
        type: "success",
        message: `Quote received: ~${ethers
          .formatEther(data.quote.amountOut)
          .slice(0, 8)} ${tokenOut.symbol}`,
      });
      setActiveStep(1);
    } catch (err: any) {
  ...
  const generateAndSubmitProof = async () => {
    if (!note || !quote || !address || !publicClient) {
      setFeedback({
        type: "error",
        message: "Missing required information for trade.",
      });
      return;
    }
    ...
    try {
      ...
      setFeedback({
        type: "info",
        message: "Preparing to generate ZK proof...",
      });
      ...
      setFeedback({
        type: "info",
        message: "Fetching deposit events for Merkle tree...",
      });
      ...
      if (leafIndex < 0)
        throw new Error("Note commitment not found in Merkle tree.");
      ...
      setFeedback({
        type: "info",
        message: "Generating ZK proof... This can take a moment.",
      });
      ...
      setFeedback({
        type: "info",
        message: "Proof generated. Submitting trade intent to relayer...",
      });
      ...
      setFeedback({
        type: "info",
        message: `Trade intent submitted! ID: ${data.intentId}. Waiting for relayer...`,
      });
    } catch (err: any) {
      setFeedback({ type: "error", message: err.message });
    ...
    try {
      ...
      switch (data.status) {
        case "submitted":
          setFeedback({
            type: "info",
            message: `Transaction submitted to Flashbots! Tx Hash: ${data.txHash.slice(
              0,
              10
            )}...`,
          });
        ...
        case "confirmed":
          setFeedback({
            type: "success",
            message: `Trade Confirmed! Tx Hash: ${data.txHash.slice(0, 10)}...`,
          });
          setActiveStep(3);
        ...
        case "failed":
          setFeedback({
            type: "error",
            message: `Trade Failed: ${data.message}`,
          });
          setIntentId(null); // Stop polling
          setActiveStep(0);
          break;
        default: // pending
          setFeedback({
            type: "info",
            message: "Intent is pending with relayer...",
          });
      }
    } catch (err: any) {
  ...
  const getButton = () => {
    if (activeStep === 0) {
      return (
        <button
          ...
          className="w-full flex items-center justify-center bg-blue-600 hover:bg-blue-700 disabled:bg-gray-500 text-white font-bold py-3 px-4 rounded-lg transition-all"
        >
    ...
    return (
      <button
        ...
        disabled={isProcessing || !!intentId}
        className="w-full flex items-center justify-center bg-blue-600 hover:bg-blue-700 disabled:bg-gray-500 text-white font-bold py-3 px-4 rounded-lg transition-all"
      >
    ...
    >
      <div className="text-center">
        ...
        <p className="text-gray-400 mt-2">
          Swap tokens privately using your deposited notes.
        </p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between space-x-2">
          ...
          <div className="w-2/3 bg-gray-900 text-gray-400 border border-gray-600 rounded-md p-3 text-right h-[48px] flex items-center justify-end">
            {quote
              ? `~${ethers.formatEther(quote.quote.amountOut).slice(0, 8)}`
              : "..."}
          </div>
        </div>
        ...
        {activeStep === 3 && (
          <button
            onClick={resetState}
            className="w-full mt-2 flex items-center justify-center bg-gray-600 hover:bg-gray-700 text-white font-bold py-3 px-4 rounded-lg transition-all"
          >
            Start New Trade
          </button>
        )}
