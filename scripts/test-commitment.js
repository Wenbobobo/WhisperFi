...
async function main() {
  ...
  const commitment =
    "0x" + poseidon.F.toObject(hash).toString(16).padStart(64, "0");
  ...
  const foundEvent = depositEvents.find(
    (event) => event.args.commitment === commitment
  );
  if (foundEvent) {
    ...
    console.log(
      "Available:",
      depositEvents.map((e) => e.args.commitment)
    );
  }
}
