...
async function testHashConsistency() {
  ...
  const testCases = [
    ...
    },
    {
      ...
      input2: BigInt(
        "4293953647789117937979294483098317889782348404923555839435689930607973263466"
      ),
    },
  ];
  ...
  for (let i = 0; i < testCases.length; i++) {
    ...
    try {
      ...
      // 合约计算
      const contractResult = await poseidonContract.poseidon([
        testCase.input1,
        testCase.input2,
      ]);
      ...
      if (!isMatch) {
        console.log(
          `   ❌ 差异: ${BigInt(frontendHash) - BigInt(contractHash)}`
        );
      }
    } catch (error) {
