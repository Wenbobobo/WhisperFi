...
template Withdraw(levels) {
    ...
    // 1. Calculate the commitment from the secret and amount
    component commitmentHasher = Poseidon(2);
    commitmentHasher.inputs[0] <== secret;
    commitmentHasher.inputs[1] <== amount;
    signal commitment <== commitmentHasher.out;

    // 2. Verify that the commitment is part of the Merkle tree
    ...
    // 3. Calculate the nullifier from the secret
    component nullifierHasher = Poseidon(1);
    nullifierHasher.inputs[0] <== secret;
    signal calculatedNullifier <== nullifierHasher.out;

    // 4. Constrain the public nullifier to be the one calculated from the secret
